...
class DyGANGenerator(BaseGenerator):                                                                    #k=100
    def __init__(self, H_inputs, batch_size, H, z_dim, H_t, N, rw_len, temp, disten=False, k=2, rm_th=0.001, add_th=0.95, device='cpu'):
        ...
        self.W_up = nn.Linear(H, N, device=self.device).type(torch.float64)
        ...
        if input_rws == None: ## For training
            ...
            with torch.autograd.profiler.record_function("gen forward: graph editing path"):
                ...
                for step in torch.arange(num_batches, device=self.device):
                    #regio DEBBUG
                    if debug_generator:
                        print(f"Verification step: {step}")
                    #endregio
                    rm_rw = []
                    add_rw = []
                    ...
                    for rw_step in torch.arange(1, self.rw_len, device=self.device): #KEY FORLOOP HERE COPIED 2 OR 3 TIMES
                        ...
                        H_graph, H_time = hh, hh
                        s = time.time()
                        with torch.autograd.profiler.record_function("gen forward: W_up in graph editing"):
                            p = self.W_up(H_graph)  
                        # logging.info(f'{rw_step} W_up {time.time() - s}')
                        #regio DEBBUG
                        if debug_generator:
                            print(f"p shape: {p.shape}")
                        #endregio
                        s = time.time()
                        with torch.autograd.profiler.record_function("gen forward: gumbel_softmax_sample in graph editing"):
                            v = self.gumbel_softmax_sample(p, self.temp).squeeze()
                        # logging.info(f'{rw_step} gumbel_softmax_sample {time.time() - s}')
                        v_true = F.one_hot(indices[rw_step], num_classes=self.N).to(self.device).double().view(1,-1)
                        ...
                        #regio DEBBUG
                        if debug_generator:
                            print(f"v shape: {v.shape}, v_true shape: {v_true.shape}, t_true shape: {t_true.shape}")
                        #endregio
                        rm_th = v.sort().values[int(len(v) * 0.05)]
                        top_candidate = torch.argmax(v)  # index
                        v_hard = F.one_hot(top_candidate, num_classes=self.N).to(self.device).double().view(1,-1)
                        if v[top_candidate] > self.add_th:
                            # Don't change the input to the next LSTM layer
                            # v_true = v_hard
                            # t_true = t
                            #regio DEBBUG
                            if debug_generator:
                            ...
                            #endregio
                            top_candidate = top_candidate.detach().item()
