...
def get_data(args, data_config, debug_directory = False):
    ...
    print(args)
    transaction_file = args.data
    df_edges = pd.read_csv(transaction_file)
    # df_edges = df_edges.drop(columns=['Embedding'])
    ...
    # if args.emb == 'pre':
    #     x = torch.load(f'/nobackup/users/sammit/model/HTNE_Pre_HI-Tiny_seconds.ckpt')['node_emb.weight'].cpu()
    # if args.emb == 'pre_new':
    #     xx = torch.load(f'/nobackup/users/sammit/model/HTNE_Pre_HI-Tiny_seconds_new.ckpt')
    #     temp = np.concatenate((xx['node_emb.weight'].cpu().data.numpy(), xx['node_emb2.weight'].cpu().data.numpy()), axis=1)
    #     x = torch.tensor(temp, dtype=torch.float)
    # if args.emb == 'pre_new128x2':
    #     xx = torch.load(f'/nobackup/users/sammit/model/HTNE_Pre_HI-Tiny_seconds_new128x2.ckpt')
    #     temp = np.concatenate((xx['node_emb.weight'].cpu().data.numpy(), xx['node_emb2.weight'].cpu().data.numpy()), axis=1)
    #     x = torch.tensor(temp, dtype=torch.float)
    # if args.emb == 'post':
    #     x = torch.load(f'/home/sammit/TGEditor/src/temp_HI-Tiny_htne.ckpt')['node_emb.weight'].cpu()
    # if args.emb == 'post_new':
    #     xx = torch.load(f'/nobackup/users/sammit/temp_HI-Tiny_htne2_new.ckpt')
    #     temp = np.concatenate((xx['node_emb.weight'].cpu().data.numpy(), xx['node_emb2.weight'].cpu().data.numpy()), axis=1)
    #     x = torch.tensor(temp, dtype=torch.float)
    # if args.emb == 'dygan_down':
    #     x = torch.load(f'/nobackup/users/sammit/temp_weight.ckpt').cpu()
    # if args.emb == 'all':
    #     x1 = torch.load(f'/nobackup/users/sammit/temp_weight.ckpt').cpu()
    #     x2 = torch.load(f'/nobackup/users/sammit/model/HTNE_Pre_HI-Tiny_seconds_new.ckpt')
    #     temp = np.concatenate((x2['node_emb.weight'].cpu().data.numpy(), x2['node_emb2.weight'].cpu().data.numpy(), x1), axis=1)
    #     x = torch.tensor(temp, dtype=torch.float)
    if args.emb == 'old':
        x = torch.load(args.emb_path)['node_emb.weight'].cpu()
    if args.emb == 'new':
        xx = torch.load(args.emb_path)
        temp = np.concatenate((xx['node_emb.weight'].cpu().data.numpy(), xx['node_emb2.weight'].cpu().data.numpy()), axis=1)
        x = torch.tensor(temp, dtype=torch.float)
    logging.info(f"Embedding shape: {x.shape}")

    # x = torch.load(f'/home/sammit/TGEditor/src/intermediate_embed.ckpt')['node_emb.weight'].cpu()
    # x = torch.load(f'/home/sammit/TGEditor/src/temp_Chase_htne.ckpt')['node_emb.weight'].cpu()
