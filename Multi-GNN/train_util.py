...
def get_loaders(tr_data, val_data, te_data, tr_inds, val_inds, te_inds, transform, args, debug_directory = False):
    if isinstance(tr_data, HeteroData):
        ...
        # seed = 1
        # # Define the number of batches you want to print
        ...
        for i, batch in enumerate(tr_loader, start=1):
            if debug_directory: logging.info(f"Batch {i}")
        
            # Print node features
            if debug_directory: logging.info(f"Node features: {batch.x}")
        
            # Print edge connections
            if debug_directory: logging.info(f"Edge connections: {batch.edge_index}")
        
            # Print edge attributes, if available
            if hasattr(batch, 'edge_attr'):
                if debug_directory: logging.info(f"Edge attributes: {batch.edge_attr}")
        
            # Print labels, if available
            if hasattr(batch, 'y'):
                if debug_directory: logging.info(f"Labels: {batch.y}")
        
            # Check if you've printed the desired number of batches
            if i >= num_batches_to_print:
                break
        # torch.save(tr_loader, os.path.join(debug_directory, 'tr_loader1.pt'))
        ...
        #     # Print node features
        #     logging.info(f"Node features: {batch.x}")
        
        #     # Print edge connections
        #     logging.info(f"Edge connections: {batch.edge_index}")
        
        #     # Print edge attributes, if available
        #     if hasattr(batch, 'edge_attr'):
        #         logging.info(f"Edge attributes: {batch.edge_attr}")
        
        #     # Print labels, if available
        #     if hasattr(batch, 'y'):
        #         logging.info(f"Labels: {batch.y}")
        
        #     # Check if you've printed the desired number of batches
        #     if i >= num_batches_to_print:
        #         break
        # torch.save(tr_loader, os.path.join(debug_directory, 'tr_loader2.pt'))

        print("len(val_data)", len(val_data))
        print("args.num_neighs", args.num_neighs)
        print("len(val_data.edge_index[:, val_inds])", len(val_data.edge_index[:, val_inds]))
...
@torch.no_grad()
def evaluate_homo(loader, inds, model, data, device, args, loss_fn):
    '''Evaluates the model performane for homogenous graph data.'''
    preds = []
    ground_truths = []
    total_loss = 0
    total_examples = 0  # Initialize total examples count
    for batch in tqdm.tqdm(loader, disable=not args.tqdm):
        ...
        with torch.no_grad():
            ...
            # Calculate loss
            loss = loss_fn(out, batch.y[mask].to(device, dtype=torch.long))
            total_loss += loss.item() * batch.y[mask].size(0)  # Accumulate scaled loss
            total_examples += batch.y[mask].size(0)  # Count examples
    
    pred = torch.cat(preds, dim=0).cpu().numpy()
    ground_truth = torch.cat(ground_truths, dim=0).cpu().numpy()
    f1 = f1_score(ground_truth, pred, average='macro')
